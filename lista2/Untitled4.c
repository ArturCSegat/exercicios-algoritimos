...
void test_hand(hand * h, hand * d){     // this function is used at the end of a game to compare the player's hands with the dealer's
    ...
    play_dealer(&(*d));                  // see function definition
    ...
    else if ((*d).total > 21){          // checks for dealer bust
        printf("the dealer busted with %d, you won", (*d).total);
        (*h).alive = 'n';
    }

    else if ((*d).total == 21){         // dealer balckjack
        printf("the dealer has blackjack, you lost");
        (*h).alive = 'n';
    }

    else if((*h).total > (*d).total){    // compares hands
        printf("You won with %d agaisnt the dealer with %d", (*h).total, (*d).total);
        (*h).alive = 'n';
    }

    else{
        printf("you lost, the dealer had %d", (*d).total);
        (*h).alive = 'n';
    }
    }
...
void play_dealer(hand * d){    // simulates the the dealer drawing in a casino (dealer draws to 17)
    ...
    if ((*d).total > 21){ // i dont thinks this does anthing
        (*d).alive = 'n';
    }
